...
def _( Shape1, Shape2 ):
    ...
    ##################################################################
    PairsCostD2 = np.zeros((m, n))
    PairsCostD1 = np.zeros((m, n))
    # 構造体としての定義必要
    ...
    PairsCost = PairsCostD1 * alpha + PairsCostD2 * (1 - alpha) 
    ...
    mn = len_shape_1*len_shape_2
    Costs = np.zeros((mn, mn))  # Costs matrix
    Aff = np.zeros((mn, mn))    # Affinity matrix
    ...
    matching = np.zeros((m, n))
    match_ind = np.zeros((mn, 1))
    done = 0
    ind_max = 0
    exclude = []
