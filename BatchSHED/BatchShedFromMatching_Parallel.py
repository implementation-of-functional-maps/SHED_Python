...
#                       Method
##################################################################
def _( Shapes1, Shapes2, Matchings, *Weights ):
    '''
    ...
    costs.costGeometry = np.zeros((m,n))
    costs.costScale = np.zeros((m,n))
    costs.costPosition = np.zeros((m,n))
    costs.costDuplicate = np.zeros((m,n))
    
    import my_modules.matlab_python.nargin
    import BatchSHED.ShedFromMatching as ShedFromMatching
    for i in range(m):
       for j in range(n):
        if my_modules.matlab_python.nargin._(Shapes1, Shapes2, Matchings, Weights ) > 2:
            # Use given weights:
            [curr_shed, cost] = ShedFromMatching._(Shapes1[i], Shapes2[j], Matchings[i][j])
        else:
            # Use default weights:
            [curr_shed, cost] = ShedFromMatching._(Shapes1[i], Shapes2[j], Matchings[i][j])
        shed[i, j] = curr_shed
